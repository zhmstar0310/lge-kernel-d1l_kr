			...
			struct mdp4_overlay_pipe *pipe)
{
	...
	if (vctrl->base_pipe != NULL)
		return 0;
/*Work-aroud code for HDMI blue screen issue while camera preview is working.
Codes are provided by QCT. And now we are debuging for root cause(SR : 01006078, CR : 420963)*/
#if defined(CONFIG_MACH_MSM8960_D1L_KR) || defined(CONFIG_MACH_MSM8960_D1L)
		{
		uint32 reg = inpdw(MDP_BASE+0xe0044);
		outpdw(MDP_BASE+0xe0044, reg|0x90000);
		}
#endif
...
Codes are provided by QCT. And now we are debuging for root cause(SR : 01006078, CR : 420963)*/
		...
		{
		uint32 reg = inpdw(MDP_BASE+0xe0044);
		outpdw(MDP_BASE+0xe0044, reg&0x60fff);
		}
#endif
	return result;
}
