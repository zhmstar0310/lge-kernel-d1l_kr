			    ...
			    boolean op_enable)
{
  ...
  */
			...
			if(mfd->index == 0){
				fbi = mfd->fbi;
				var = &fbi->var;
				var->left_margin = msm_fb_pdata->porch[0];
				var->right_margin = msm_fb_pdata->porch[1];
				var->upper_margin = msm_fb_pdata->porch[3];
				var->lower_margin = msm_fb_pdata->porch[4];

				printk("%s, hbp = %d, hfp = %d, vbp = %d, vfp = %d\n",__func__,
